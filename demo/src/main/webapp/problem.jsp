...
<div class="page page-dashboard clearfix" ng-controller="DashboardCtrl">
    ...
    <script>
        layui.use(["jquery", "layer", "table", "layedit", "form", "laydate", "upload", "carousel"], function () {
            ...
            function a() {
                var data = getAjax("/problem/getList")
                table.render({
                    elem: '#test9'
                    , data: data
                    , title: '用户数据表'
                ...
                if (layEvent === "upl") {
                    layer.open({
                        ...
                        content: '                <form action="" style="margin: 10px 10px"  class="layui-form">' +
                            ...
                            '                            <input type="text" name="title" value="'+data.title+'" required lay-verify="required" maxlength="20" placeholder="请输入..." autocomplete="off" class="layui-input">' +
                            '                        </div>' +
                            '                    </div>' +
                            '                    <div class="layui-form-item">' +
                            '                        <label class="layui-form-label">内容</label>' +
                            '                        <div class="layui-input-block">' +
                            '    <textarea name="content" class="layui-textarea" lay-verify="required">'+data.content+'</textarea>' +
                            '                        </div>' +
                            '                    </div>' +
                    ...
                    form.on("submit(formVerify)", function (da) {
                        da.field["id"]=data.id
                        getAjax1(JSON.stringify(da.field), "/problem/save")
                        layer.msg("修改成功")
                        layer.closeAll()
                        a()
                        return false;
                    ...
                    getAjax1(JSON.stringify(data), "/problem/del")
                    layer.msg("删除成功")
                    a()
                }
            });
            ...
            $("#add").on("click", function () {
                layer.open({
                    ...
                    content: '                <form action="" style="margin: 10px 10px"  class="layui-form">' +
                        ...
                        '                        <label class="layui-form-label">标题</label>' +
                        '                        <div class="layui-input-block">' +
                        '                            <input type="text" name="title" required lay-verify="required" maxlength="20" placeholder="请输入..." autocomplete="off" class="layui-input">' +
                        '                        </div>' +
                        '                    </div>' +
                        '                    <div class="layui-form-item">' +
                        '                        <label class="layui-form-label">内容</label>' +
                ...
                form.on("submit(formVerify)", function (data) {
                    getAjax1(JSON.stringify(data.field), "/problem/save")
                    layer.msg("保存成功")
                    layer.closeAll()
                    a()
                    return false;
